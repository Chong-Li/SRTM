...
import (
	...
	"github.com/WU-CPSL/RTM-0.1/nsqio/Test/publisher_mtc/mq"
)
...
func main() {
	...
	numTest, _ := strconv.Atoi(os.Args[1]) // number of goroutines
	numTopic, _ := strconv.Atoi(os.Args[2])
	gap, _ := strconv.Atoi(os.Args[3])
	for i := 0; i < numTopic; i++ {
		for j := 0; j < numTest; j++ {
			go newTest(13001, 1000, strconv.Itoa(i), gap, os.Args[4], os.Args[5])
		}
	}
	for {
		time.Sleep(50 * time.Second)
