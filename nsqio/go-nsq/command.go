...
/*func (c *Command) WriteTo(w io.Writer) (int64, error) {
	...
	msg := make([]byte, len(c.Name))
	copy(msg, c.Name)

	for _, param := range c.Params {
		msg = append(msg, byteSpace...)
		msg = append(msg, param...)
	}

	msg = append(msg, byteNewLine...)
...
// RTM: Report Local Traffic Rate Statistics to lookupd
func ReportRate(params [][]byte) *Command {
	return &Command{[]byte("RATE"), params, nil, ""}
}

// RTM: Report CPU Statistics to lookupd
func ReportCPU(params [][]byte) *Command {
	return &Command{[]byte("CPU"), params, nil, ""}
}

// RTM: Report the new topic's timelines to lookupd
func ReportTime(params [][]byte) *Command {
	return &Command{[]byte("TIME"), params, nil, ""}
}

// RTM: Report all topics' timelines to lookupd
func ReportTP(params [][]byte) *Command {
	return &Command{[]byte("TP"), params, nil, ""}
}

// RTM: Back new rates from lookupd
func Back(params [][]byte) *Command {
	return &Command{[]byte("BACK"), params, nil, ""}
}
...
// jarry not added yet
// Nack create a new command to indicate that
// a given message (by id) is missing.
func Nack(id uint64) *Command {
	var params = [][]byte{[]byte(strconv.Itoa(int(id)))}
	return &Command{[]byte("NACK"), params, nil, ""}
}
