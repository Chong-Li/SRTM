...
type Consumer struct {
	...
	mTCPPeerConns      map[string][]*Conn //RTM.dtb
	conntypes          map[string]int     // jarry add connection type for each connection
...
func (r *Consumer) conns() []*Conn {
	r.mtx.RLock()
	conns := make([]*Conn, 0, len(r.connections)+len(r.mTCPPeerConns)) //RTM.dtb
	for addr, c := range r.connections {
		// only return TCP connection
		if r.conntypes[addr] == ConnTypeTCP {
			conns = append(conns, c)
		} else if r.conntypes[addr] == ConnTypeMTCP {
			conns = append(conns, c)
			for _, x := range r.mTCPPeerConns[addr] {
...
type lookupResp struct {
	...
	MTCAddress string      `json:"mtcaddress"`
...
func (r *Consumer) queryLookupd() {
	...
	var connTypes []int
	if r.channel == "multicast" && data.MTCAddress != "" {
		nsqdAddrs = append(nsqdAddrs, data.MTCAddress)
		connTypes = append(connTypes, ConnTypeMulticast)
	} else if strings.HasPrefix(r.channel, "mtcp") { //RTM.dtb
		for _, producer := range data.Producers {
			broadcastAddress := producer.BroadcastAddress
			port := producer.TCPPort
			joined := net.JoinHostPort(broadcastAddress, strconv.Itoa(port))
			nsqdAddrs = append(nsqdAddrs, joined)
			connTypes = append(connTypes, ConnTypeMTCP)
		}
		...
		for _, producer := range data.Producers {
			...
			joined := net.JoinHostPort(broadcastAddress, strconv.Itoa(port))
			nsqdAddrs = append(nsqdAddrs, joined)
			connTypes = append(connTypes, ConnTypeDefault)
		}
	}
	// apply filter
	// jarry: imcomplete remove filter for now. need to apply filter to conntypes as well.
	//if discoveryFilter, ok := r.behaviorDelegate.(DiscoveryFilter); ok {
	//	nsqdAddrs = discoveryFilter.Filter(nsqdAddrs)
	//}
	for i, addr := range nsqdAddrs {
		if connTypes[i] == ConnTypeMTCP {
			r.mtx.RLock()
			if r.connections[addr]!=nil && len(r.mTCPPeerConns[addr])+1==r.config.MTCPConn {
				r.mtx.RUnlock()
				continue
			}
			r.mtx.RUnlock()
			for index := 0; index < r.config.MTCPConn; index++ {
				err = r.ConnectToNSQDMTCP(addr, ConnTypeMTCP, index)
			...
			err = r.ConnectToNSQD(addr, connTypes[i])
			if err != nil && err != ErrAlreadyConnected {
				r.log(LogLevelError, "(%s) error connecting to nsqd - %s", addr, err)
				continue
			}
...
func (r *Consumer) ConnectToNSQDs(addresses []string) error {
	for _, addr := range addresses {
		err := r.ConnectToNSQD(addr, ConnTypeDefault)
		if err != nil {
			return err
		}
	}
...
//RTM.dtb
func (r *Consumer) ConnectToNSQDMTCP(addr string, connType int, index int) error {
	if atomic.LoadInt32(&r.stopFlag) == 1 {
		return errors.New("consumer stopped")
	}
	...
	conn := NewConn(addr, connType, &r.config, &consumerConnDelegate{r})
	conn.SetLogger(logger, logLvl,
		fmt.Sprintf("%3d [%s/%s] (%%s)", r.id, r.topic, r.channel))

	r.mtx.Lock()
	r.pendingConnections[addr] = conn
	if idx := indexOf(addr, r.nsqdAddrs); idx == -1 {
		r.nsqdAddrs = append(r.nsqdAddrs, addr)
	}
	r.mtx.Unlock()
	...
	cmd := Subscribe(r.topic, strconv.Itoa(index)+r.channel)
	...
	delete(r.pendingConnections, addr)
	if index == 0 {
		r.conntypes[addr] = connType
		r.connections[addr] = conn
	} else {
		r.mTCPPeerConns[addr] = append(r.mTCPPeerConns[addr], conn)
	}
...
// instance.
func (r *Consumer) ConnectToNSQD(addr string, connType int) error {
	if atomic.LoadInt32(&r.stopFlag) == 1 {
		return errors.New("consumer stopped")
	}
	...
	// jarry only send subscribe when conntype is TCP
...
func (r *Consumer) DisconnectFromNSQD(addr string) error {
	...
	idx := indexOf(addr, r.nsqdAddrs)
	if idx == -1 {
		return ErrNotConnected
	}

	// slice delete
	r.nsqdAddrs = append(r.nsqdAddrs[:idx], r.nsqdAddrs[idx+1:]...)
...
func (r *Consumer) onConnMessage(c *Conn, msg *Message) {
	if c.connType == ConnTypeDefault || c.connType == ConnTypeMTCP {
		...
		if bytes.HasPrefix(msg.Body[24:], []byte{'0'}) {
			binary.PutVarint(msg.Body[100:], time.Now().UnixNano())
			lenByte := []byte(strconv.Itoa(len(r.incomingMessages)))
			lenByte = append(lenByte, "\n"...)
			copy(msg.Body[120:], lenByte)
		}
...
func (r *Consumer) onConnMessageNacked(c *Conn, msg *Message) {
	atomic.AddUint64(&r.messagesNacked, 1)
}
...
func (r *Consumer) onConnClose(c *Conn) {
	...
	// jarry remove conntype for the connection
	delete(r.conntypes, c.String())
	...
	r.mtx.RLock()
	numLookupd := len(r.lookupdHTTPAddrs)
	reconnect := indexOf(c.String(), r.nsqdAddrs) >= 0
	r.mtx.RUnlock()
	if numLookupd > 0 {
		...
		go func(addr string, connType int) {
			for {
				...
				r.mtx.RLock()
				reconnect := indexOf(addr, r.nsqdAddrs) >= 0
				r.mtx.RUnlock()
				if !reconnect {
					r.log(LogLevelWarning, "(%s) skipped reconnect after removal...", addr)
					return
				}
				err := r.ConnectToNSQD(addr, connType)
				if err != nil && err != ErrAlreadyConnected {
					r.log(LogLevelError, "(%s) error connecting to nsqd - %s", addr, err)
					continue
				}
...
func (r *Consumer) maybeUpdateRDY(conn *Conn) {
	// do not run if connection is multicast
	if conn.connType != ConnTypeMulticast {
...
func (r *Consumer) handlerLoop(handler Handler) {
	...
	for {
		...
		if r.shouldFailMessage(message, handler) {
			fmt.Println("fail message for ", message.Attempts)
