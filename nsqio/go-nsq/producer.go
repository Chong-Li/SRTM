...
type Producer struct {
	...
	//RTM
	msgQ     map[string]chan *ProducerTransaction
	congest  map[string]bool
	pullChan chan int
	index    int
	topics   []string
	//congestLock map[string]*sync.RWMutex
...
func NewProducer(addr string, config *Config) (*Producer, error) {
	...
	p := &Producer{
		...
		//RTM
		msgQ:    make(map[string]chan *ProducerTransaction),
		congest: make(map[string]bool),
		//congestLock: make(map[string]*sync.RWMutex),
...
func (w *Producer) sendCommandAsync(cmd *Command, doneChan chan *ProducerTransaction,
	...
	//RTM
	topic := string(t.cmd.Params[0][:])
	if w.msgQ[topic] == nil {
		w.msgQ[topic] = make(chan *ProducerTransaction, 5000)
		w.congest[topic] = false
		//w.congestLock[topic] = new(sync.Mutex)
		w.topics = append(w.topics, topic)
	}

	//RTM.dtb, for profiling stage
	/*w.congestLock.RLock()
	congest := w.congest[topic]
	w.congestLock.RUnlock()
	if congest == true {
		return nil
	}*/
	select {
...
func (w *Producer) connect() error {
	...
	// jarry Producer connect to Daemon with TCP
	w.conn = NewConn(w.addr, ConnTypeDefault, &w.config, &producerConnDelegate{w})
...
//RTM
func (w *Producer) congestHandler(topic string) {
	w.congestLock.Lock()
	w.congest[topic] = true
	w.congestLock.Unlock()
}

//RTM
func (w *Producer) decongestHandler(topic string) {
	w.congestLock.Lock()
	w.congest[topic] = false
	w.congestLock.Unlock()
	go func() {
...
func (w *Producer) router() {
	for {
		...
		case <-w.pullChan:
			for i := 0; i < len(w.topics); i++ {
				if len(w.msgQ[w.topics[w.index]]) > 0 && !w.congest[w.topics[w.index]] {
					t := <-w.msgQ[w.topics[w.index]]
					w.transactions = append(w.transactions, t)
					//RTM.dtb
					if bytes.HasPrefix(t.cmd.Body[24:], []byte{'0'}){
						binary.PutVarint(t.cmd.Body[0:], time.Now().UnixNano())
					}
					err := w.conn.WriteCommand(t.cmd)
					if err != nil {
						w.log(LogLevelError, "(%s) sending command - %s", w.conn.String(), err)
		...
		case data := <-w.responseChan:
			...
			} else if bytes.Equal(data[:7], []byte("CONGEST")) {
				//fmt.Printf("Receive response CONGEST%s\n", string(data[7:]))
				w.congestHandler(string(data[7:]))
			} else if bytes.Equal(data[:9], []byte("DECONGEST")) {
				//fmt.Printf("Receive response DECONGEST%s\n", string(data[9:]))
				w.decongestHandler(string(data[9:]))
			}
...
func (w *Producer) nextLookupdEndpoint(command string, data string) string {
	...
	if command == "producer_lookup" {
		...
		v.Add("priority", params[0])
		v.Add("topic", params[1])
		v.Add("pub", params[2])
		hostname, _ := os.Hostname()
		v.Add("hostname", hostname)
		u.RawQuery = v.Encode()
	} else if command == "producer_lookup_v3" {
		if u.Path == "/" || u.Path == "" {
			u.Path = "/producer_lookup_v3"
		}
		v, _ := url.ParseQuery(u.RawQuery)
		params := strings.Split(data, "\n")
		v.Add("priority", params[0])
...
func (w *Producer) ConnectToNSQLookupd_v2(addr string, priority string, topic string, pubID string) error {
	if atomic.LoadInt32(&w.stopFlag) == 1 {
		return errors.New("producer stopped")
	}
...
//RTM
func (w *Producer) ConnectToNSQLookupd_v3(addr string, priority string, topic string, pubID string) error {
	if atomic.LoadInt32(&w.stopFlag) == 1 {
		return errors.New("producer stopped")
	}
	...
	// if this is the first one, kick off the go loop
	if numLookupd == 1 {
		w.addr = w.queryLookupd_v3(priority + "\n" + topic + "\n"+ pubID)
	}
	return nil
}

func (w *Producer) queryLookupd_v3(params string) string {
	//find a nsqd with the appropriate params
	endpoint := w.nextLookupdEndpoint("producer_lookup_v3", params)
	w.log(LogLevelInfo, "querying nsqlookupd v3 %s", endpoint)
	var data lookupResp
	err := apiRequestNegotiateV1("GET", endpoint, nil, &data)
	if err != nil {
		w.log(LogLevelError, "error querying nsqlookupd (%s) - %s", endpoint, err)
...
func (w *Producer) queryLookupd_v2(params string) string {
	//find a nsqd with the appropriate params
	endpoint := w.nextLookupdEndpoint("producer_lookup_v2", params)
	...
	var nsqdAddrs []string
	for _, producer := range data.Producers {
		joined := net.JoinHostPort(producer.BroadcastAddress, strconv.Itoa(producer.TCPPort))
		nsqdAddrs = append(nsqdAddrs, joined)
	}
