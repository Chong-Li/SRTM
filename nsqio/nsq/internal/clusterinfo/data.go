...
import (
	...
	"github.com/WU-CPSL/RTM-0.1/nsqio/nsq/internal/stringy"
...
//jarry no finished.
// Get the address for each channel. address information
func (c *ClusterInfo) GetLookupdTopicChannelAddresses(topic string, channel string, lookupdHTTPAddrs []string) ([]string, error) {
	// var connTypes []int
	...
	// encode conntype into address ???
	type respType struct {
		//ConnTypes []int    `json:"conntypes"`
		Addresses []string `json:"addresses"` // in the forme of  1,224.0.0.1:8000. connType, address:port.
	}
	...
	for _, addr := range lookupdHTTPAddrs {
		...
		go func(addr string) {
			...
			endpoint := fmt.Sprintf("http://%s/addresses?topic=%schannel=%s", addr, url.QueryEscape(topic), url.QueryEscape(channel))
			c.logf("CI: querying nsqlookupd %s", endpoint)
			...
			lock.Lock()
			defer lock.Unlock()
			//connTypes = append(connTypes, resp.ConnTypes...)
	...
	addresses = stringy.Uniq(addresses)
	sort.Strings(addresses)
	//for _, address range addresses {
	// not finished, need to parse the address for different connType
	//}
	if len(errs) > 0 {
		return addresses, ErrList(errs)
	}
