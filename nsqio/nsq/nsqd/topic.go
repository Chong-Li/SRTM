	...
	for {
		...
		if reserve < t.Tmpindex {
				...
				if bytes.HasPrefix(msg.Body[24:], []byte{'0'}) {
					...
					/*then1, _:=binary.Varint(msg.Body[40:58])
                                        strx := []byte(strconv.Itoa(int(time.Now().UnixNano()-then1)))
                                        strx = append(strx, "\n"...)
                                        strx = append(strx, []byte(strconv.Itoa(int(then1-msg.Ktimestamp.UnixNano())))...)
                                        strx = append(strx, "\n"...)
                                        copy(msg.Body[60:], strx)*/

                                        /*strx := []byte(strconv.Itoa(int(time.Now().UnixNano()- msg.IdealTime)))
                                        strx = append(strx, "\n"...)
                                        copy(msg.Body[60:], strx)*/
		...
		if t.congestion {
			...
			if len(t.memMsgChan) > 0 {
				...
				msg = <- t.memMsgChan
				r1 := t.bucket.ReserveN(msg.Ktimestamp, 1)
				// RTM:
				...
				//if r1delay <= 0 {
					//RTM.dtb test only
					if bytes.HasPrefix(msg.Body[24:], []byte{'0'}) {
						//curx:=time.Now().UnixNano()
						binary.PutVarint(msg.Body[60:], time.Now().UnixNano())
						//binary.PutVarint(msg.Body[40:], curx)
						//strx := []byte(strconv.Itoa(0))
						/*then1, _:=binary.Varint(msg.Body[40:58])
                                        	strx := []byte(strconv.Itoa(int(time.Now().UnixNano()-then1)))
						strx = append(strx, "\n"...)
						strx = append(strx, []byte(strconv.Itoa(int(then1-msg.Ktimestamp.UnixNano())))...)
						...
						/*strx := []byte(strconv.Itoa(int(r1delay)))
						strx = append(strx, "\n"...)
						copy(msg.Body[60:], strx)*/
					}
					if t.congest &&
				...
				t.Tmpindex=0
				t.Tmpq[t.Tmpindex]=msg
				t.Tmpindex++
				reserve = len(t.memMsgChan)
				if reserve >= 1 {
					if reserve > t.quantum-1 {
						reserve = t.quantum-1
					}
					for x:= 0; x<reserve; x++ {
						t.Tmpq[t.Tmpindex]=<-t.memMsgChan
						t.Tmpindex++
					}
					rN := t.bucket.ReserveN(t.Tmpq[t.Tmpindex-1].Ktimestamp, reserve)
					delay = rN.Delay()
					//delay = rN.DelayFrom(t.Tmpq[t.Tmpindex-1].Ktimestamp)
				} else {
					delay = r1.Delay()
					//delay = r1delay
				}
				...
				//x1Flag = true
				//adj:=time.Now().Sub(msg.Ktimestamp)
				//time.Sleep(delay+adj)
				if bytes.HasPrefix(msg.Body[24:], []byte{'0'}) {
