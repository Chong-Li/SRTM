...
import (
	...
	"github.com/WU-CPSL/RTM-0.1/nsqio/nsq/internal/app"
...
func nsqFlagset() *flag.FlagSet {
	...
	flagSet.String("broadcast-address", "127.0.0.1", "<addr> address to registered with lookupd (defaults to the OS hostname)")
	flagSet.Int64("connection-type", 0, "connection type, 0 to be TCP, 1 to be multicast (default to TCP)")
	flagSet.String("interface", "eth0", "interface to use for communication (only important for multicast)")
	...
	flagSet.Int64("max-rdy-count", 10000, "maximum RDY count for a client")
	...
	flagSet.Duration("msg-safe-timeout", 100*time.Millisecond, "message removed after safe-timeout")
	flagSet.Duration("max-rtt", 5*time.Millisecond, "maximum rtt assumption for multicast clients")
	flagSet.Duration("last-send-timeout", 2*time.Millisecond, "time to send null message")
	flagSet.Int64("max-null-send", 2, "number of null messages to send")
	flagSet.Duration("nack-resend-timeout", 5*time.Millisecond, "time to resend a nack if no message received")
	flagSet.Duration("max-hard-timeout", 300*time.Millisecond, "do not retransmit after max-hard-timeout")
